namespace :db do
  ...
  desc "create astroprofil"
  task astroprofils: :environment do
    users = User.all
    users.each do |user|
      Astroprofil.profil(user) unless user.sign
    end
  end

  desc "create affinites"
  task affinities: :environment do
